...
// The previous use of fmin in this file caused issues on
// some Windows setups due to fmin being in the C99 standard
// and not the default C standard library for VS 2008
#define cmaesMIN(X,Y) (X) < (Y) ? (X) : (Y)
		...
		double const *lower_bounds, double const *upper_bounds, unsigned long len_of_bounds)
{
	...
	for(i = 0; i < t->len_of_bounds; ++i) {
		...
		t->al[i] = cmaesMIN((ub[i] - lb[i]) / 2., (1. + fabs(lb[i])) / 20.);
		t->au[i] = cmaesMIN((ub[i] - lb[i]) / 2., (1. + fabs(ub[i])) / 20.);
	}
}
